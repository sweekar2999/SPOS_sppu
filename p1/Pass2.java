...
class TableRow {
    ...
    public TableRow(String name, int address, int serial) {
        this.name = name;
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (parts[0].contains("AD") || parts[0].contains("DL,02")) {
                ...
                if (parts[0].contains("DL")) { // DC INSTR
                    ...
                    if (opcode == 10) {
                        if (parts[1].contains("S")) {
                            ...
                            code = String.format("%02d", opcode) + "\t0\t" + String.format("%03d", SYMTAB.get(symIndex - 1).address) + "\n";
                            bw.write(code);
                        } else if (parts[1].contains("L")) {
                            int symIndex = Integer.parseInt(parts[1].replaceAll("[^0-9]", ""));
                            code = String.format("%02d", opcode) + "\t0\t" + String.format("%03d", LITTAB.get(symIndex - 1).address) + "\n";
                            bw.write(code);
                        }
                    }
                }
                ...
                if (parts[2].contains("S")) {
                    ...
                    code = String.format("%02d", opcode) + "\t" + regcode + "\t" + String.format("%03d", SYMTAB.get(symIndex - 1).address) + "\n";
                    bw.write(code);
                } else if (parts[2].contains("L")) {
                    int symIndex = Integer.parseInt(parts[2].replaceAll("[^0-9]", ""));
                    code = String.format("%02d", opcode) + "\t" + regcode + "\t" + String.format("%03d", LITTAB.get(symIndex - 1).address) + "\n";
                    bw.write(code);
                }
            }
        }
